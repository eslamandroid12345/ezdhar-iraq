...
trait DefaultImage{
    public function storeDefaultImage($path,$name){
        ...
        $imageName = "storage/uploads/$path/".time().base64_encode(time().'-'.base64_encode($name)).'.png';
        if (!file_exists(public_path("storage/uploads/$path"))) {
            mkdir(public_path("storage/uploads/$path"), 0777, true);
        }
        file_put_contents($imageName, $img->encode());
        return $imageName;
    ...
    public function uploadFiles($folder_name,$file,$is_updated_file=null){
        ...
        if ($file){
            ...
            if (!file_exists(public_path("storage/uploads/$folder_name"))) {
                mkdir(public_path("storage/uploads/$folder_name"), 0777, true);
            }
            $file->move("storage/uploads/$folder_name", $fileNameToStore);
        }else{
            $fileNameToStore=$is_updated_file;
        }
